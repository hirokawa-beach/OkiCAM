...
class LayoutManager {
    ...
    init() {
        ...
        // ドラッグイベントのリスナー登録（PC用）
        this.overviewContainer.addEventListener('dragstart', (e) => this.handleDragStart(e));
        this.overviewContainer.addEventListener('dragover', (e) => this.handleDragOver(e));
        this.overviewContainer.addEventListener('drop', (e) => this.handleDrop(e));
        this.overviewContainer.addEventListener('dragend', (e) => this.handleDragEnd(e));
        
        // タッチイベントのリスナー登録（スマホ用）
        this.overviewContainer.addEventListener('touchstart', (e) => this.handleTouchStart(e), { passive: false });
        this.overviewContainer.addEventListener('touchmove', (e) => this.handleTouchMove(e), { passive: false });
        this.overviewContainer.addEventListener('touchend', (e) => this.handleTouchEnd(e));
        
        // 非表示ボタンのリスナー登録
        this.overviewContainer.addEventListener('click', (e) => this.handleToggleVisibility(e));

        // ロケーションボタンのリスナー登録
        this.locationBtn.addEventListener('click', () => this.toggleLocationDialog());
        
        // ロケーションダイアログの表示切り替えリスナー
        if (this.locationDialog) {
            this.locationDialog.addEventListener('change', (e) => this.handleLocationToggle(e));
            this.locationDialog.addEventListener('click', (e) => this.handleLocationDialogClick(e));
        }
     ...
     */
    initializeAllItems() {
        const items = this.overviewContainer.querySelectorAll('.overview > [class]');
        items.forEach(item => {
    ...
    handleDragOver(e) {
        ...
        if (container && container !== this.draggedElement) {
            ...
            if (e.clientY < midpoint) {
                ...
                container.parentNode.insertBefore(this.draggedElement, container.nextSibling);
            }
        }
    }
    ...
    handleTouchStart(e) {
        ...
        if (e.target.classList.contains('drag-handle')) {
            ...
            this.touchDragClone.style.left = touch.clientX - container.offsetWidth / 2 + 'px';
    ...
    handleTouchMove(e) {
        ...
        if (targetContainer && targetContainer !== this.touchDragElement) {
            ...
            if (touch.clientY < midpoint) {
                targetContainer.parentNode.insertBefore(this.touchDragElement, targetContainer);
            } else {
                targetContainer.parentNode.insertBefore(this.touchDragElement, targetContainer.nextSibling);
            }
    ...
    showFilterView(region) {
        ...
        // PC用ドラッグイベント
        itemsContainer.addEventListener('dragstart', (e) => this.handleFilterViewDragStart(e));
        itemsContainer.addEventListener('dragover', (e) => this.handleFilterViewDragOver(e, itemsContainer));
        itemsContainer.addEventListener('drop', (e) => this.handleFilterViewDrop(e));
        itemsContainer.addEventListener('dragend', (e) => this.handleFilterViewDragEnd(e));
        
        // スマホ用タッチイベント
        itemsContainer.addEventListener('touchstart', (e) => this.handleTouchStart(e), { passive: false });
        itemsContainer.addEventListener('touchmove', (e) => this.handleTouchMove(e), { passive: false });
        itemsContainer.addEventListener('touchend', (e) => this.handleTouchEnd(e));
        
        itemsContainer.addEventListener('click', (e) => this.handleFilterViewToggleVisibility(e));
    ...
    handleFilterViewDragOver(e, itemsContainer) {
        ...
        const container = e.target.closest('[data-regions]');
        if (container && container !== this.draggedElement && itemsContainer.contains(container)) {
            const rect = container.getBoundingClientRect();
            const midpoint = rect.top + rect.height / 2;
    ...
    handleFilterViewToggleVisibility(e) {
        if (!e.target.classList.contains('toggle-visibility-btn')) {
            return;
        }
    ...
    syncFilterViewChangesToOverview() {
        ...
        const filterItems = this.filterViewContainer.querySelectorAll('[data-regions]');
        const originalItems = [];
